...
import { updatePostLog } from "@/lib/supabase/postLogsQueries";
...
export default function PostLogEditForm({ log, onCancel, onSaved }: Props) {
  ...
  const [imagePreviewUrl, setImagePreviewUrl] = useState<string | null>(log.imageUrl);
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      const image = imageStored ? { stored: imageStored } : imageRemoved ? "remove" as const : "keep" as const;
      const updated = await updatePostLog(user.id, log.id, content.trim(), genre, image);
      onSaved(updated);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {imagePreviewUrl ? (
        <div className="relative w-32 rounded-lg overflow-hidden" style={{ aspectRatio: "1/1" }}>
          <Image src={imagePreviewUrl} alt="" fill className="object-cover" unoptimized />
          <button
            type="button"
            onClick={removeImage}
        ...
        <label
          className="flex items-center gap-1.5 w-fit px-3 py-2 rounded-lg text-xs font-medium cursor-pointer transition-opacity hover:opacity-80"
